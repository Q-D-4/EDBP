...
أثناء التثبيت ينشأ حساب فني اسمه `edbpadmin`. وعند أول إقلاع من القرص يشغّل النظام إعداد OOBE لإنشاء اسم الجهاز وحساب المستخدم اليومي العادي. التثبيت نفسه مصمم ليكتمل من محتوى الـISO دون مستودع APT على الشبكة.

السياسات الأمنية الأساسية تشمل خطًا عامًا لجدار `nftables` يحظر الاتصالات الواردة غير المطلوبة، وUSBGuard، وتعطيل Bluetooth، وSSH بمفتاح عام لحساب `edbpadmin` فقط. التقسيم يبقى تفاعليًا حتى يختار الفني القرص والخطة ويؤكد الكتابة بنفسه.
...
يحتاج البناء إلى مدخلين محليين لحساب `edbpadmin`:
...
### 4.1 إنشاء hash كلمة مرور `edbpadmin`
...
chmod 600 secrets/edbpadmin_password_hash
```
...
### 4.2 إنشاء مفتاح SSH لحساب `edbpadmin`
...
- **المفتاح العام** ينتهي عادةً بـ`.pub`. يمكن وضعه في مدخل البناء ليصبح سطرًا داخل `authorized_keys` لحساب `edbpadmin`.
...
ssh-keygen -t ed25519 -a 100 \
    -f "$HOME/.ssh/edbp_edbpadmin_ed25519" \
    -C "edbp-edbpadmin"
```
...
```text
$HOME/.ssh/edbp_edbpadmin_ed25519       ← PRIVATE: لا تنسخه إلى المشروع
$HOME/.ssh/edbp_edbpadmin_ed25519.pub   ← PUBLIC: هذا الذي يسمح بنسخه
```
...
```bash
install -m 600 \
    "$HOME/.ssh/edbp_edbpadmin_ed25519.pub" \
...
```bash
ssh-keygen -lf "$HOME/.ssh/edbp_edbpadmin_ed25519.pub"
stat -c 'file=%n owner=%U:%G mode=%a bytes=%s' \
    secrets/edbpadmin_authorized_keys
```

يقبل `scripts/stage-admin-keys` مفاتيح `ssh-ed25519` و`sk-ssh-ed25519@openssh.com` الصحيحة فقط، بحد أقصى 20 مفتاحًا. يتجاهل الأسطر الفارغة والتعليقات. بعد التثبيت يضع النظام المفاتيح العامة المقبولة في `/home/edbpadmin/.ssh/authorized_keys`، بينما يولد لكل جهاز مفاتيح SSH host فريدة.

> **تحذير حاسم:** لا تنسخ `$HOME/.ssh/edbp_edbpadmin_ed25519` إلى `secrets/`. لا ترفعه إلى GitHub، ولا ترسله إلى جهاز الهدف. الملف المسموح به في مدخل البناء هو الملف الذي ينتهي بـ`.pub` فقط.
...
```bash
test -d secrets
test -f secrets/edbpadmin_password_hash
test ! -L secrets/edbpadmin_password_hash
test -f secrets/edbpadmin_authorized_keys
test ! -L secrets/edbpadmin_authorized_keys
...
- `admin key source must be...` أو `no SSH public keys...`: أنشئ `secrets/edbpadmin_authorized_keys` من المفتاح العام فقط وأعد `chmod 600`.
- `edbpadmin password hash file is missing`: أنشئ `secrets/edbpadmin_password_hash` بالطريقة التفاعلية في القسم 4.1.
...
6. ينشأ `edbpadmin` من hash البناء، وتثبت مفاتيحه العامة، وتولد مفاتيح SSH host فريدة، وتفعل خدمات EDBP المطلوبة.
7. بعد اكتمال target واختيار `Finish the installation`، تظهر شاشة Debian القياسية `Installation complete`. عندما تطلب الرسالة إزالة وسيط التثبيت، انزع USB حتى لا يقلع الجهاز منه مرة أخرى.
8. اضغط `Continue`. يكمل installer الإنهاء ويعيد تشغيل الجهاز بصورة طبيعية، ثم يقلع EDBP من القرص المثبت ويبدأ OOBE.
...
- من 1 إلى 63 محرفًا.
- أحرف إنجليزية صغيرة أو أرقام أو شرطات داخلية.
- لا يبدأ الاسم بشرطة ولا ينتهي بها.

أمثلة صحيحة: `workstation-01`، `lab-node-9`، `a`.

أمثلة مرفوضة: `Workstation-01`، `workstation.example`، `-workstation`، `workstation-`.
...
- الطول الكلي من 1 إلى 32 محرفًا.
- يبدأ الاسم بحرف إنجليزي صغير.
- بقية المحارف أحرف صغيرة أو أرقام أو شرطة أو underscore.
- لا مسافات ولا نقاط.
- الاسم `edbpadmin` محجوز، وأي اسم مستخدم أو مجموعة موجودة مسبقًا مرفوض.

أمثلة صحيحة: `alex`، `service_user`، `helpdesk-2`.

أمثلة مرفوضة: `2alex`، `Alex`، `alex.smith`، `edbpadmin`.
...
`edbpadmin` حساب الفني أو مسؤول الطوارئ، وليس حساب العمل اليومي. خصائصه:

- يستطيع تشغيل `sudo`، وتبقى كلمة المرور مطلوبة.
- يستطيع تسجيل الدخول محليًا بكلمة المرور التي ولّد المشغل hash لها وقت البناء.
- SSH متاح له بالمفاتيح العامة التي وضعت في `secrets/edbpadmin_authorized_keys`؛ تسجيل SSH بكلمة مرور معطل.
...
المستخدم الذي ينشئه OOBE منفصل: لا يحصل على `sudo` ولا يسمح له إعداد SSH الحالي بتسجيل الدخول عن بعد. استخدم `edbpadmin` للصيانة المضبوطة، والمستخدم العادي للأعمال اليومية.

## 13. الشبكة والسياسات الأمنية

### الشبكة و`nftables`

التثبيت نفسه offline. بعد الإقلاع المثبت يدير NetworkManager اتصال Ethernet أو Wi-Fi. يشغّل EDBP خطًا عامًا مضبوطًا لجدار `nftables` بسياسة drop للحركة الجديدة الواردة والصادرة ولتمرير الحزم. يسمح بالـloopback والاتصالات المنشأة، ويسمح بـICMP وSSH ذي المفتاح العام من نطاقات IPv4 وIPv6 الخاصة وlink-local المعرفة بالمعايير، ويسمح بالحركة الصادرة الجديدة إلى تلك النطاقات فقط. لا يسمح الخط العام بالإنترنت العام.

هذه النطاقات معيارية ولا تمثل شبكة جهة بعينها. لا يتضمن الخط العام subnet خاصًا بالنشر، أو host، أو domain، أو اسم interface، أو استثناء تشغيلي. إذا استخدم النشر عناوين عامة أو احتاج DHCP أو service discovery أو خدمات واردة إضافية أو egress مختلفًا، فاستبدل `/etc/nftables.conf` بسياسة downstream منفصلة ومراجعة. تحقق من السياسة النشطة بالأوامر:
...
أبقِ `nftables.service` مفعّلًا، ووثّق أي سياسة خاصة بالموقع خارج EDBP العام.
...
- SSH يسمح بالمفتاح العام لحساب `edbpadmin` فقط؛ root وكلمات مرور SSH وkeyboard-interactive وforwarding معطلة.
...
**المشكلة:** يفشل التحقق من `edbpadmin_authorized_keys` أو `edbpadmin_password_hash`.
...
**الحل:** استخدم نموذجًا مثل `workstation-01` للجهاز و`alex` للمستخدم، مع الحدود المذكورة في القسم 11.
...
> - لا تنشر hash كلمة مرور `edbpadmin` بلا حاجة؛ يمكن استخدامه في تخمين offline.
