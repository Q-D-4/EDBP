...
## EDBP original wallpaper

### `config/includes.chroot_after_packages/usr/share/wallpapers/EDBP/contents/images/1920x1080.jpg`

The wallpaper is original work created entirely by Osama Haddad (Q-D-4).

Copyright © 2026 Osama Haddad (Q-D-4).

Applicable license: `CC-BY-SA-4.0`. The complete license text is provided at `LICENSES/CC-BY-SA-4.0.txt`.

### `config/includes.chroot_after_packages/usr/share/wallpapers/EDBP/metadata.json`

This EDBP-authored metadata and configuration file remains under the
repository default `GPL-3.0-or-later` license. Its JSON field
`"License": "CC-BY-SA-4.0"` describes the wallpaper asset and does not
relicense `metadata.json` itself.
